...
func TestSearchCandidates_FilterByIndustry(t *testing.T) {
	...
	uID, _, _ := testutil.CreateCompanyUser(db, "si@ex.com", "si", "pass123", "Search Ind Co", true)
	tok := testutil.GenerateToken(uID.String(), "company", 15*time.Minute)

	_, p1, _ := testutil.CreateJobseeker(db, "si1@ex.com", "si1", "pass123", "Ind One")
	_, p2, _ := testutil.CreateJobseeker(db, "si2@ex.com", "si2", "pass123", "Ind Two")

	db.Exec(`INSERT INTO job_experiences (id, profile_id, type, title, organization, start_date, is_current, industry, skills_used) VALUES ($1,$2,'employment','Go Dev','Tech Co','2020-01',true,$3,$4)`,
		"33333333-3333-3333-3333-333333333333", p1.String(), "Technology", `{Go,PostgreSQL}`)
	db.Exec(`INSERT INTO job_experiences (id, profile_id, type, title, organization, start_date, is_current, industry, skills_used) VALUES ($1,$2,'employment','Nurse','Hosp A','2019-06',true,$3,$4)`,
...
func TestSearchCandidates_Combined(t *testing.T) {
	...
	uID, _, _ := testutil.CreateCompanyUser(db, "scb@ex.com", "scb", "pass123", "Search Comb Co", true)
	tok := testutil.GenerateToken(uID.String(), "company", 15*time.Minute)

	_, p1, _ := testutil.CreateJobseeker(db, "cb1@ex.com", "cb1", "pass123", "Senior Go Dev")
	_, p2, _ := testutil.CreateJobseeker(db, "cb2@ex.com", "cb2", "pass123", "Junior React Dev")

	db.Exec(`INSERT INTO job_experiences (id, profile_id, type, title, organization, start_date, is_current, industry, skills_used) VALUES ($1,$2,'employment','Go Dev','Tech Co','2020-01',true,$3,$4)`,
		"55555555-5555-5555-5555-555555555555", p1.String(), "Technology", `{Go,PostgreSQL}`)
	db.Exec(`INSERT INTO job_experiences (id, profile_id, type, title, organization, start_date, is_current, industry, skills_used) VALUES ($1,$2,'employment','React Dev','Web Co','2019-06',true,$3,$4)`,
...
func TestSearchCandidates_Pagination(t *testing.T) {
	...
	uID, _, _ := testutil.CreateCompanyUser(db, "sp@ex.com", "sp", "pass123", "Search Pag Co", true)
	tok := testutil.GenerateToken(uID.String(), "company", 15*time.Minute)
	...
	for i := 1; i <= 3; i++ {
		...
		_, pID, _ := testutil.CreateJobseeker(db, email, user, "pass123", name)
		db.Exec(`INSERT INTO job_experiences (id, profile_id, type, title, organization, start_date, is_current, skills_used) VALUES ($1,$2,'employment','Dev','Co','2020-01',true,$3)`,
...
func TestSearchCandidates_InvalidQueryParams(t *testing.T) {
	...
	uID, _, _ := testutil.CreateCompanyUser(db, "siq@ex.com", "siq", "pass123", "Search Inv Co", true)
	tok := testutil.GenerateToken(uID.String(), "company", 15*time.Minute)
...
func TestSearchCandidates_UnverifiedCompany(t *testing.T) {
	...
	uID, _, _ := testutil.CreateCompanyUser(db, "uv@ex.com", "uv", "pass123", "Unverified Co", false)
	tok := testutil.GenerateToken(uID.String(), "company", 15*time.Minute)
